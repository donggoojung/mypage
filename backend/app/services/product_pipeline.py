...
# 2026-09-22 scripts/find_shoe_category.py로 실API 직접 확인됨: "나이키 에어포스1 운동화",
# "아디다스 스탠스미스 운동화", "나이키 운동화" 등 명확한 신발 상품명으로 쿠팡 카테고리
# 자동추천 API를 호출하면 68010이 나오고, 그 고시정보가 실제로 "구두/신발"로 조회된다
# (이전에 넣었던 56137은 실제로는 "화장품", 52021은 실제로는 "도서" 카테고리로 연결되는
# 잘못된 값이었다 — 둘 다 검증 없이 넣었다가 실사용 중 발견되어 교체함). 정확한 카테고리가
# 중요하면 고급 옵션에서 display_category_code를 직접 지정해야 한다.
DEFAULT_DISPLAY_CATEGORY_CODE = 68010
