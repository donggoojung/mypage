...
def test_refresh_syncs_newly_sold_out_size_to_coupang(db_session):
    ...
    listing = MarketListing(
        ...
        selling_price=Decimal("194000"),  # 이미 최신 계산가와 같게 둬서 가격변경은 안 일어나게 함
...
def test_refresh_syncs_price_increase_to_coupang(db_session):
    ...
    # 원가 139000원은 10~15만원 구간(매입가 대비 목표마진 20% = 27,800원) + 택배비 4000원 반영,
    # 1,000원 단위 올림 결과.
    assert listing.selling_price == Decimal("194000")
